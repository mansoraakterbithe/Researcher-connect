    ...
    const fetchPosts = async () => {
      try {
        ...
        const mapped = (data.posts || []).map((p) => ({
          ...
          time: new Date(p.createdAt).toLocaleDateString("en-GB", {
            day: "numeric",
            month: "short",
            year: "numeric",
          }),
    ...
    <div className="hf">
      ...
      <nav className="hf-nav">
        ...
        <div className="hf-nav-right">
          ...
          <div className="hf-nav-link" onClick={() => navigate("/connections")}>
            Connections
          </div>
          <div className="hf-nav-link" onClick={() => navigate("/messages")}>
            Messages
          </div>
          <div
          ...
          <div className="hf-avatar-wrap" ref={avatarMenuRef}>
            <div
              className="hf-avatar"
            ...
            {showAvatarMenu && (
              <div className="hf-avatar-menu">
                ...
                <div
                  className="hf-avatar-menu-item logout"
                  onClick={() => {
                    logout();
                    navigate("/auth");
                    setShowAvatarMenu(false);
      ...
      <div className="hf-body">
        ...
        <aside className="hf-sidebar">
          ...
          {[
            ...
            {
              icon: "M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07A19.5 19.5 0 0 1 4.69 12 19.79 19.79 0 0 1 1.61 3.35 2 2 0 0 1 3.6 1h3a2 2 0 0 1 2 1.72c.127.96.361 1.903.7 2.81a2 2 0 0 1-.45 2.11L7.91 8.6a16 16 0 0 0 6 6l.94-.94a2 2 0 0 1 2.11-.45c.907.339 1.85.573 2.81.7A2 2 0 0 1 22 16.92z",
              label: "Messages",
              path: "/messages",
            },
          ].map((item) => (
            <div
              key={item.label}
              className={`hf-side-item ${item.active ? "active" : ""}`}
              ...
              <div className="hf-post-wrap" key={post.id}>
                ...
                >
                  <div className="hf-post-header">
                    <div className="hf-post-author">
                      ...
                      <div>
                        <div className="hf-post-name">
                          {post.author}
                          {post.verified && (
                            <span className="hf-verified">✓ Verified</span>
                          )}
                        </div>
                    ...
                    {post.match && (
                      <div className="hf-match">⚡ {post.match}% match</div>
                    )}
                  </div>
                  ...
                  <div className="hf-post-footer">
                    <div className="hf-post-tags">
                      {post.tags.map((tag) => (
                        <span
                          key={tag.label}
                          className={`hf-tag hf-tag-${tag.color}`}
                        >
                          {tag.label}
                        </span>
                      ))}
                    ...
                    <div className="hf-post-actions">
                      {post.type === "opportunity" ? (
                        <button className="hf-action hf-action-apply">
                          Apply Now
                        </button>
                      ) : (
        ...
        <aside className="hf-right">
          <div className="hf-widget">
            ...
            <div className="hf-profile-mini">
              ...
              <div className="hf-profile-name">
                {user?.username || "Researcher"}
              </div>
              <div className="hf-profile-role">
                {user?.role || "Student"} · ResearchConnect
              </div>
            </div>
            ...
            {TRENDING.map((t) => (
              <div className="hf-trend" key={t.label}>
                <div className="hf-trend-label">
                  <div
                    className="hf-trend-dot"
                    style={{ background: t.color }}
                  />
                  {t.label}
                </div>
                <div className={`hf-trend-pct ${t.up ? "" : "down"}`}>
                  {t.pct}
                </div>
              </div>
            ))}
